...
var $author$project$LinearSearch$GotRandomArray = function (a) {
	return {$: 'GotRandomArray', a: a};
};
var $author$project$LinearSearch$Increment = {$: 'Increment'};
...
var $author$project$LinearSearch$msgToString = function (m) {
	if (m.$ === 'Increment') {
		return 'Increment';
	} else {
		return '';
...
var $author$project$LinearSearch$init = function (_v0) {
	return _Utils_Tuple2(
		{
			ctrls: _List_fromArray(
				[
					{
					label: $author$project$LinearSearch$msgToString($author$project$LinearSearch$Increment),
					message: $author$project$LinearSearch$Increment,
					status: $author$project$Controls$Active,
					text: 'Click increment to scan the first item.'
				}
				]),
			iter: -1,
		...
		A2(
			$elm$random$Random$generate,
			$author$project$LinearSearch$GotRandomArray,
			A3(
...
var $author$project$LinearSearch$subscriptions = function (model) {
	return $elm$core$Platform$Sub$none;
};
var $author$project$LinearSearch$GotTarget = function (a) {
...
var $author$project$LinearSearch$select = F2(
	function (i, _v0) {
		...
		if (!_v1) {
			var pm = function () {
				var _v2 = _Utils_eq(target, ci);
				if (_v2) {
					return _Utils_Tuple2(
						'Search Target found at index ' + ($elm$core$String$fromInt(i) + ' of the List.'),
						$author$project$Prompt$PromptSuccess);
				} else {
					return _Utils_Tuple2(
						'The item at index ' + ($elm$core$String$fromInt(i) + (' has number ' + ($elm$core$String$fromInt(ci) + ('. It does not match the search target of ' + $elm$core$String$fromInt(target))))),
			...
			return _Utils_Tuple2(
				selections,
				_Utils_Tuple2(
					'The item at index ' + ($elm$core$String$fromInt(i) + (' has number ' + ($elm$core$String$fromInt(ci) + '. It has already been scanned.'))),
...
var $author$project$LinearSearch$increment = function (model) {
	var _v0 = model;
	var numbers = _v0.numbers;
	var selections = _v0.selections;
	var ctrls = _v0.ctrls;
	...
	var _v1 = _Utils_Tuple2(
		_Utils_cmp(
			iter + 1,
			$elm$core$List$length(numbers)) < 0,
		!_Utils_eq(
			A2(
				$elm$core$Maybe$withDefault,
				-1,
		...
		return _Utils_update(
			model,
			{
				ctrls: A3(
					...
					$author$project$LinearSearch$Increment,
					'Target element already found, no need to iterate more',
					A2($author$project$Controls$disable, $author$project$LinearSearch$Increment, ctrls))
			});
	} else {
		if (_v1.a) {
			var _v2 = A2($author$project$LinearSearch$select, iter + 1, model);
			var selections_ = _v2.a;
			var pm = _v2.b;
			return _Utils_update(
				model,
				{
					ctrls: A3($author$project$Controls$updateText, $author$project$LinearSearch$Increment, 'Click Increment to scan next item.', ctrls),
					iter: iter + 1,
					prompt: pm,
			...
			return _Utils_update(
				model,
				{
					ctrls: A3(
						$author$project$Controls$updateText,
						$author$project$LinearSearch$Increment,
						'Iterated whole array, ' + ($elm$core$String$fromInt(target) + ' is not a member of the List'),
						A2($author$project$Controls$disable, $author$project$LinearSearch$Increment, ctrls))
				});
		}
	}
...
var $author$project$LinearSearch$update = F2(
	function (msg, model) {
		var _v0 = model;
		var numbers = _v0.numbers;
		var selections = _v0.selections;
		...
		switch (msg.$) {
			case 'Increment':
				return _Utils_Tuple2(
					$author$project$LinearSearch$increment(model),
			...
			case 'GotRandomArray':
				...
				return _Utils_Tuple2(
					...
					A2(
						$elm$random$Random$generate,
						$author$project$LinearSearch$GotTarget,
						A2(
...
var $author$project$LinearSearch$NoMsg = {$: 'NoMsg'};
...
var $author$project$LinearSearch$numberToBGColor = F4(
	function (num, idx, selections, target) {
		var sel_idx = A2(
			$elm$core$Maybe$withDefault,
			-1,
			A2($elm_community$list_extra$List$Extra$getAt, 0, selections));
		var label_ = function () {
			var _v2 = _Utils_eq(idx, sel_idx);
			if (_v2) {
...
var $author$project$LinearSearch$viewNums = F3(
	function (nums, selections, target) {
		var msg = function (ci) {
			return $author$project$LinearSearch$NoMsg;
		};
		var lwidth = 1100;
		return A2(
			$elm$html$Html$div,
					...
					_List_fromArray(
						[
							...
							A2(
								...
								F2(
									function (pos, n) {
										return A4($author$project$LinearSearch$numberToBGColor, n, pos, selections, target);
									}),
								nums),
							selections,
							msg)
...
var $author$project$LinearSearch$viewSearchTarget = function (target) {
	return A2(
		$elm$html$Html$span,
		_List_fromArray(
			[
...
var $author$project$LinearSearch$view = function (model) {
	var _v0 = model;
	var numbers = _v0.numbers;
	var ctrls = _v0.ctrls;
	var selections = _v0.selections;
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$author$project$LinearSearch$viewSearchTarget(target)
					])),
				A2(
				$elm$html$Html$div,
				_List_fromArray(
				...
				_List_fromArray(
					[
						A3($author$project$LinearSearch$viewNums, numbers, selections, target)
					])),
				A2(
				$elm$html$Html$div,
				_List_fromArray(
				...
				_List_fromArray(
					[
						A2($author$project$Controls$view, ctrls, $author$project$LinearSearch$msgToString)
					]))
			]));
};
var $author$project$LinearSearch$main = $elm$browser$Browser$element(
	{init: $author$project$LinearSearch$init, subscriptions: $author$project$LinearSearch$subscriptions, update: $author$project$LinearSearch$update, view: $author$project$LinearSearch$view});
_Platform_export({'LinearSearch':{'init':$author$project$LinearSearch$main(
