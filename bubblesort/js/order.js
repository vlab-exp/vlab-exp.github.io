...
var $author$project$Order$init = function (_v0) {
	return _Utils_Tuple2(
		{
			numbers: _List_Nil,
...
var $author$project$Order$subscriptions = function (model) {
		...
		switch (selections.$) {
			...
			case 'OneSelected':
				...
				return _Utils_eq(i, j) ? _Utils_Tuple2(
					$author$project$BSCore$NoneSelected,
					_Utils_Tuple2('Select any two numbers to order.', $author$project$Core$Prompt$PromptInfo)) : _Utils_Tuple2(
			...
			default:
				...
				return _Utils_eq(i, j) ? _Utils_Tuple2(
					...
					_Utils_Tuple2('You need exactly two numbers.  Select another one.', $author$project$Core$Prompt$PromptInfo)) : (_Utils_eq(i, k) ? _Utils_Tuple2(
					$author$project$BSCore$OneSelected(j),
...
var $author$project$Order$outOfOrder = F4(
	function (i, j, x, y) {
		var c2 = (_Utils_cmp(i, j) < 0) && (_Utils_cmp(x, y) > 0);
		var c1 = (_Utils_cmp(i, j) > 0) && (_Utils_cmp(x, y) < 0);
		return c1 || c2;
	});
var $author$project$Order$ordered = F3(
	function (array, i, j) {
		var aj = A2($elm_community$list_extra$List$Extra$getAt, j, array);
		var ai = A2($elm_community$list_extra$List$Extra$getAt, i, array);
		var _v0 = _Utils_Tuple2(ai, aj);
		if ((_v0.a.$ === 'Just') && (_v0.b.$ === 'Just')) {
			var x = _v0.a.a;
			var y = _v0.b.a;
			return !A4($author$project$Order$outOfOrder, i, j, x, y);
		} else {
			return false;
		}
	});
var $author$project$Order$select = F3(
	function (i, selections, numbers) {
		switch (selections.$) {
			case 'NoneSelected':
				return _Utils_Tuple2(
					$author$project$BSCore$OneSelected(i),
					_Utils_Tuple2('Select one more number and then click the \'Order\' button', $author$project$Core$Prompt$PromptSuccess));
			case 'OneSelected':
				var j = selections.a;
				return _Utils_Tuple2(
					A2($author$project$BSCore$BothSelected, j, i),
					A3($author$project$Order$ordered, numbers, i, j) ? _Utils_Tuple2('Select numbers are in correct order.  Select some other numbers.', $author$project$Core$Prompt$PromptInfo) : _Utils_Tuple2('The selected numbers are out of order.  Click the \'Order\' button to order the selected numbers.', $author$project$Core$Prompt$PromptSuccess));
			default:
				var j = selections.a;
				var k = selections.b;
				return _Utils_Tuple2(
...
var $author$project$Order$swap = F2(
...
var $author$project$Order$order = F2(
	function (array, selections) {
		if (selections.$ === 'BothSelected') {
			...
			if ((_v1.a.$ === 'Just') && (_v1.b.$ === 'Just')) {
				var x = _v1.a.a;
				var y = _v1.b.a;
				return A4($author$project$Order$outOfOrder, i, j, x, y) ? A2($author$project$Order$swap, array, selections) : array;
			} else {
				return array;
			}
		} else {
...
var $author$project$Order$updateOrder = function (model) {
	var _v0 = model;
	...
	switch (selections.$) {
		case 'NoneSelected':
			return _Utils_update(
				model,
				{
					prompt: _Utils_Tuple2('Select two numbers and then click the \'Order\' button.', $author$project$Core$Prompt$PromptInfo)
				});
		case 'OneSelected':
			var i = selections.a;
			return _Utils_update(
		...
		default:
			var i = selections.a;
			var j = selections.b;
			return A3($author$project$Order$ordered, numbers, i, j) ? model : _Utils_update(
				model,
				{
...
var $author$project$Order$update = F2(
	function (msg, model) {
		var _v0 = model;
		var numbers = _v0.numbers;
		var selections = _v0.selections;
		switch (msg.$) {
			case 'Order':
				return _Utils_Tuple2(
					$author$project$Order$updateOrder(model),
					$elm$core$Platform$Cmd$none);
			case 'Select':
				var x = msg.a;
				var _v2 = A3($author$project$Order$select, x, selections, numbers);
			...
			case 'Deselect':
				var i = msg.a;
				var _v3 = A2($author$project$Order$deselect, i, selections);
...
var $author$project$Order$Deselect = function (a) {
	return {$: 'Deselect', a: a};
};
var $author$project$Order$Select = function (a) {
	return {$: 'Select', a: a};
};
...
var $author$project$Order$viewNums = F2(
	function (nums, selections) {
		var msg = function () {
			switch (selections.$) {
				case 'NoneSelected':
					return function (ci) {
						return $author$project$Order$Select(ci);
					};
				case 'OneSelected':
					var i = selections.a;
					return function (ci) {
						return _Utils_eq(ci, i) ? $author$project$Order$Deselect(ci) : $author$project$Order$Select(ci);
					};
				default:
					var i = selections.a;
					var j = selections.b;
					return function (ci) {
						return (_Utils_eq(ci, i) || _Utils_eq(ci, j)) ? $author$project$Order$Deselect(ci) : $author$project$Order$Select(ci);
					};
			}
		}();
		var lwidth = 1100;
					...
					_List_fromArray(
						[
							...
							A2(
								$elm$core$List$map,
								function (n) {
									return _Utils_Tuple2(n, $elm$core$Maybe$Nothing);
								},
								nums),
							$author$project$BSCore$selectionsList(selections),
							msg)
						]))
				]));
	});
var $author$project$Order$view = function (model) {
	var _v0 = model;
	var numbers = _v0.numbers;
	var selections = _v0.selections;
	var prompt = _v0.prompt;
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						A2($author$project$Order$viewNums, numbers, selections)
					])),
				A2(
				$elm$html$Html$div,
				_List_fromArray(
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$Events$onClick($author$project$Order$Order),
