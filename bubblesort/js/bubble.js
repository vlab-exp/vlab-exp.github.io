...
var $author$project$Bubble$setFresh = function (msg) {
	if (msg.$ === 'Init') {
		return true;
	} else {
		return false;
...
var $author$project$Bubble$subscriptions = function (model) {
...
var $author$project$BSCore$deselect = F2(
	function (i, selections) {
		switch (selections.$) {
			case 'NoneSelected':
				return $author$project$BSCore$NoneSelected;
			case 'OneSelected':
				var j = selections.a;
				return _Utils_eq(i, j) ? $author$project$BSCore$NoneSelected : $author$project$BSCore$OneSelected(j);
			default:
				var j = selections.a;
				var k = selections.b;
...
var $author$project$BSCore$outOfOrder = F4(
	function (i, j, x, y) {
		var c2 = (_Utils_cmp(i, j) < 0) && (_Utils_cmp(x, y) > 0);
		var c1 = (_Utils_cmp(i, j) > 0) && (_Utils_cmp(x, y) < 0);
		return c1 || c2;
	});
...
var $author$project$BSCore$swap = F2(
	function (array, selections) {
		switch (selections.$) {
			case 'NoneSelected':
				return array;
			case 'OneSelected':
			...
			default:
				var i = selections.a;
				var j = selections.b;
				return A3($elm_community$list_extra$List$Extra$swapAt, i, j, array);
		}
	});
var $author$project$BSCore$order = F2(
	function (array, selections) {
		if (selections.$ === 'BothSelected') {
			var i = selections.a;
			var j = selections.b;
			var aj = A2($elm_community$list_extra$List$Extra$getAt, j, array);
			var ai = A2($elm_community$list_extra$List$Extra$getAt, i, array);
			var _v1 = _Utils_Tuple2(ai, aj);
			if ((_v1.a.$ === 'Just') && (_v1.b.$ === 'Just')) {
				var x = _v1.a.a;
				var y = _v1.b.a;
				return A4($author$project$BSCore$outOfOrder, i, j, x, y) ? A2($author$project$BSCore$swap, array, selections) : array;
			} else {
				return array;
			}
		} else {
...
var $author$project$BSCore$adjOrder = F2(
	function (array, selection) {
		if (selection.$ === 'Just') {
			var i = selection.a;
			return A2(
				$author$project$BSCore$order,
				array,
				A2($author$project$BSCore$BothSelected, i, i + 1));
		} else {
			return array;
		}
	});
var $author$project$BSCore$increment = F2(
	function (array, index) {
		return (_Utils_cmp(
			index,
			$elm$core$List$length(array) - 1) < 0) ? _Utils_Tuple2(
			A2(
				$author$project$BSCore$adjOrder,
				array,
				$elm$core$Maybe$Just(index)),
			index + 1) : _Utils_Tuple2(array, index);
	});
var $elm$core$Platform$Cmd$none = $elm$core$Platform$Cmd$batch(_List_Nil);
var $author$project$Bubble$selects = F2(
	function (ci, nums) {
		return _Utils_eq(
			ci,
		...
		switch (msg.$) {
			case 'Increment':
				...
				var selections_ = A2(
					...
					iter,
					A2($author$project$Bubble$selects, iter_, numbers_));
				switch (selections_.$) {
					case 'BothSelected':
						var i = selections_.a;
						var j = selections_.b;
...
var $author$project$ListView$txt = F3(
	function (x, y, v) {
		return A2(
			$elm$svg$Svg$text_,
			_List_fromArray(
...
var $author$project$ListView$cell = F8(
	function (x, y, v, idx, label, selected, inplace, m) {
		var idxCrc = selected ? A6($author$project$ListView$crc_, '#fff', '#F44336', 0.5, $author$project$ListView$cell_radius / 2, 0, $author$project$ListView$cell_radius * 2) : A6($author$project$ListView$crc_, '#fff', '#fff', 1, $author$project$ListView$cell_radius / 2, 0, $author$project$ListView$cell_radius * 2);
		var cfn = function () {
			var _v1 = _Utils_Tuple2(selected, inplace);
			if (_v1.a) {
				if (_v1.b) {
					return A4($author$project$ListView$crc_, '#AED581', $author$project$ListView$strokeColor, 0.5, $author$project$ListView$cell_radius);
				} else {
					return A4($author$project$ListView$crc_, $author$project$ListView$regularFill, $author$project$ListView$strokeColor, 0.5, $author$project$ListView$cell_radius);
				}
			} else {
				if (_v1.b) {
					return A4($author$project$ListView$crc_, '#AED581', '#fff', 1, $author$project$ListView$cell_radius);
				} else {
					return A4($author$project$ListView$crc_, $author$project$ListView$regularFill, '#fff', 1, $author$project$ListView$cell_radius);
				}
			}
		}();
		return A2(
			...
			_List_fromArray(
				[
					A2(cfn, 0, 0),
					idxCrc,
					A3($author$project$ListView$txt, 0, 0, v),
					A4(
					...
					function () {
					if (label.$ === 'Nothing') {
						return A4($author$project$ListView$txt_, 0, $author$project$ListView$cell_radius * 3, 25, '');
					} else {
						var l = label.a;
						return A4($author$project$ListView$txt_, 0, $author$project$ListView$cell_radius * 3, 25, l);
					}
				}()
				]));
	});
...
var $author$project$ListView$cells = F3(
	function (nums, labelledSelections, m) {
		var sortedNums = $elm$core$List$sort(nums);
		var selections = A2(
			$elm$core$List$map,
			function (ls) {
				return ls.v;
								...
								$elm$core$List$head(
									A2(
										...
										A2(
											...
											function (ls) {
												return _Utils_eq(ls.v, p);
											},
											labelledSelections)))),
							A2($elm$core$List$member, p, selections),
							_Utils_eq(
								A2(
									$elm$core$Maybe$withDefault,
									-1,
									A2($elm_community$list_extra$List$Extra$getAt, p, sortedNums)),
								n),
							m(p));
					}),
				nums));
	});
var $author$project$Bubble$selectionsListWithLabels = function (selections) {
	switch (selections.$) {
		case 'NoneSelected':
			return _List_Nil;
		case 'OneSelected':
		...
		default:
			...
			return _List_fromArray(
				[
					...
					{l: $elm$core$Maybe$Nothing, v: j}
				]);
	}
};
var $elm$svg$Svg$svg = $elm$svg$Svg$trustedNode('svg');
var $elm$svg$Svg$Attributes$viewBox = _VirtualDom_attribute('viewBox');
var $author$project$Bubble$viewNums = F2(
	function (nums, selections) {
		var msg = function (ci) {
			return $author$project$Bubble$NoMsg;
		};
		var lwidth = 1100;
		return A2(
			$elm$html$Html$div,
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							A3(
							$author$project$ListView$cells,
							nums,
							$author$project$Bubble$selectionsListWithLabels(selections),
							msg)
						]))
				]));
	});
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						A2(
						$author$project$Bubble$viewNums,
						numbers,
						A2($author$project$Bubble$selects, iter, numbers))
					])),
				A2(
				$elm$html$Html$div,
				_List_fromArray(
...
var $author$project$Bubble$main = $elm$browser$Browser$element(
	{
		init: A3($author$project$Core$init, $elm$core$Basics$identity, $author$project$AnalyticsPort$analytics, $author$project$Bubble$init),
		subscriptions: $author$project$Core$subscriptions($author$project$Bubble$subscriptions),
		update: A6($author$project$Core$update, $elm$core$Basics$identity, $author$project$AnalyticsPort$analytics, $author$project$Bubble$update, $author$project$Bubble$setFresh, $elm$core$Maybe$Nothing, $elm$core$Maybe$Nothing),
		view: $author$project$Core$view($author$project$Bubble$view)
	});
_Platform_export({'Bubble':{'init':$author$project$Bubble$main(
