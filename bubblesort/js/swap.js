...
var $author$project$Swap$subscriptions = function (model) {
...
var $author$project$Random_list$getNum = function (num) {
	if (num.$ === 'Just') {
		var x = num.a;
		return x;
	} else {
...
var $author$project$Random_list$minSwap = function (lst) {
	if ($elm$core$List$isEmpty(lst)) {
		return 0;
	} else {
		var sortList = $elm$core$List$sort(lst);
		return _Utils_eq(
			$elm$core$List$head(sortList),
			$elm$core$List$head(lst)) ? (0 + $author$project$Random_list$minSwap(
			A2($elm$core$List$drop, 1, lst))) : (1 + $author$project$Random_list$minSwap(
			A2(
				$elm$core$List$drop,
				1,
				A3(
					...
					0,
					$author$project$Random_list$getNum(
						A2(
...
var $author$project$Swap$update = F2(
	function (msg, model) {
		...
		switch (msg.$) {
			case 'Swap':
				return _Utils_Tuple2(
					$author$project$Swap$swap(model),
					$elm$core$Platform$Cmd$none);
			case 'Select':
				var x = msg.a;
				var _v2 = A2($author$project$Swap$select, x, selections);
			...
			case 'Deselect':
				var i = msg.a;
				var _v3 = A2($author$project$Swap$deselect, i, selections);
				var selections_ = _v3.a;
				var pm = _v3.b;
				return _Utils_Tuple2(
					_Utils_update(
						model,
			...
			default:
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							minSwap: $author$project$Random_list$minSwap(
...
var $author$project$Swap$Select = function (a) {
	return {$: 'Select', a: a};
};
...
var $author$project$Swap$selectionsListWithLabels = function (selections) {
	switch (selections.$) {
		case 'NoneSelected':
			return _List_Nil;
		case 'OneSelected':
...
var $author$project$Swap$viewNums = F2(
	function (nums, selections) {
		var msg = function () {
			switch (selections.$) {
				case 'NoneSelected':
					return function (ci) {
						return $author$project$Swap$Select(ci);
					};
				case 'OneSelected':
					var i = selections.a;
					return function (ci) {
						return _Utils_eq(ci, i) ? $author$project$Swap$Deselect(ci) : $author$project$Swap$Select(ci);
					};
				default:
					var i = selections.a;
					var j = selections.b;
					return function (ci) {
						return (_Utils_eq(ci, i) || _Utils_eq(ci, j)) ? $author$project$Swap$Deselect(ci) : $author$project$Swap$Select(ci);
					};
			}
		}();
		var lwidth = 1100;
				...
				[
					...
					_List_fromArray(
						[
							...
							nums,
							$author$project$Swap$selectionsListWithLabels(selections),
							msg)
						]))
				]));
	});
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						A2($author$project$Swap$viewNums, numbers, selections)
					])),
				A2(
				$elm$html$Html$div,
				_List_fromArray(
